...
export const getArticleComments =
  commentsAdapter.getSelectors<StateScheme>(
    (state) =>
...
const articleDetailsCommentsSlice = createSlice({
  ...
  initialState:
    commentsAdapter.getInitialState<ArticelDetailsCommentsSchema>(
      {
        entities: {},
        ids: [],
        error: undefined,
  ...
  extraReducers: (builder) => {
    builder
      .addCase(
        fetchCommentsByArticleById.pending,
        (state) => {
      ...
      .addCase(
        fetchCommentsByArticleById.rejected,
        (state, action) => {
          state.isLoading = false;
